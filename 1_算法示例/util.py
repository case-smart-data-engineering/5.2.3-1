...
def draw_box(img, pos1, pos2, text, conf, color, width):
    ...
    text_len = len(text)
    img = img.copy()
    img = cv2.rectangle(img, pos1, pos2, color, thickness=3,lineType=cv2.LINE_AA)
    img = cv2.rectangle(img, (int(pos1[0]-width/2), pos1[1]-15), (int(pos1[0]+text_len*5.5+24), pos1[1]), color, -1)
...
def draw_bbox(img, bbox):
    ...
    for _, row in bbox_df.iterrows():
        ...
        #p = int(row['p'])
        p = -1
        img = draw_box(img, (x1, y1), (x2, y2), name, float(row['p']), color_dic[name], p)
    return img
...
def calculate_acc(output, target, p=.1, iou_shresh=.5):
    ...
    output_bbox = label2bbox(output).detach().cpu().numpy()
    target_bbox = label2bbox(target).detach().cpu().numpy()
